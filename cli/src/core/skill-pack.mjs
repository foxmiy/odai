...
export function selectSkillReferences({ task = "", mode = "agent_loop", includeGovernance = false } = {}) {
  ...
  if (includeGovernance) {
    refs.add("references/dao.md");
    refs.add("references/verification.md");
  }
  ...
  if (/日报|周报|工作总结|干系人更新|commit\s*(?:message|文案)|PR\s*文案|README|项目指南|整理[^，。]{0,8}文档/i.test(text)) {
    refs.add("references/craft.md");
  }
  if (
  ...
  if (mode === "subagent" && !refs.has("references/support.md")) {
    refs.add("references/support.md");
  }
  return [...refs];
}
