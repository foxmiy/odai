...
assert.ok(skillPack.supportFiles.includes("references/dao.md"));
const skillFreshnessRoot = await mkdtemp(path.join(tmpdir(), "odai-cli-skill-freshness-"));
await mkdir(path.join(skillFreshnessRoot, "skills", "odai", "references"), { recursive: true });
await writeFile(
...
await writeFile(
  path.join(skillFreshnessRoot, "skills", "odai", "references", "dao.md"),
  "# dao\nfirst reference marker\n",
  "utf8",
);
const firstSkillPack = await loadSkillPack({ repoRoot: skillFreshnessRoot });
const firstPromptPack = await firstSkillPack.render({ references: ["references/dao.md"] });
...
const secondPromptPack = await secondSkillPack.render({ references: ["references/dao.md"] });
...
const packageFallbackPromptPack = await packageFallbackSkillPack.render({
  references: ["references/dao.md", "references/verification.md"],
});
const rootVerification = await readFile(
  path.join(repoRoot, "skills", "odai", "references", "verification.md"),
  "utf8",
);
