...
7. 道未清前可做搜索、阅读、证据补强和文档起草；只有道已清、术已稳、边界明确，且用户已在本任务中至少确认过一次当前理解时，才进入代码、配置、脚本、测试与构建等实施动作。进入后默认把本轮范围内能做的动作全部执行到结果总结，不停在路线建议、单份草案、阶段交接或“已切到某个模块”处。
...
10. 若环境支持写文件且任务需要沉淀，优先续写同一份 Markdown 主文件，不开平行版本；用户说“继续”时，优先恢复已有主文件、当前主层和下一步，而不是要求重讲背景。
...
**前置判断**：用户已显式点名模块且指定了任务对象（如“按 review-sslb 审这个 diff”“用 ribao 整理 commit message”）→ 直接落到目标模块，由模块内交互契约兜底，不经过下面的决策树。
