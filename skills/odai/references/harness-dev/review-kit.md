...
- 当前主要矛盾是规划时，切到 `feature-plan`；只需完整继承它的提问强度、文档结构和“规划说明 + AI 任务草案”产物要求时，记作 `fp-strict`。
- 当前主要矛盾是设计说明，或任务涉及 UI/UX、一致性、状态体验、design system / 组件库 / token 复用判断时，切到 `design-spec`；只需完整继承它的设计校准、文档结构和交付边界时，记作 `ds-strict`。
- 当前主要矛盾是正式审查、广范围排查或用户明确要求 sslb 输出时，切到 `review-sslb`；只需完整继承它的范围解析、输出顺序、疑似未使用文件筛查和评定表时，记作 `strict-sslb`。
- 目标模块的规则优先于 workflow 的压缩展示规则。内部等价执行与严格模式继承只能基于本会话已实际读取过的模块规则；未读取就声称“按其规则继承”属于编造——要么实读目标模块，要么如实说明未读、仅按本 workflow 自身规则执行，且结果总结不得列入未读模块。`implement-code` 不适用本条的退化路径：进入实施一律实读。
