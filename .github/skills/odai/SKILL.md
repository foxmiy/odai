...
2. `道` 统一裁决：默认先读 `references/modules/dao.md`（对外称 `道`），由它根据用户语义和想法判断走哪个模块、产出什么形态；不得跳过 `道` 直接按表层关键词路由。用户明确点名模块且指定了任务对象时（如“按 review-sslb 审这个 diff”、“用 ribao 整理 commit message”），可直接落到目标模块，由模块内交互契约兜底；`道` 不为此类直达任务额外补问。
